...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        ...
        await altron.edit(f"_⚡️𝐊ɪsᴋɪ 𝐆ᴀᴀɴᴅ 𝐌ᴀᴀʀɴɪ 𝐁ᴏss⚡️_\n» `{mp} ms`")
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"😖ʀᴇʙᴏᴏᴛ ᴅᴏɴᴇ ᴊᴜsᴛ ᴄᴀʟʟ ᴘʀᴏғᴇssᴏʀ ᴅᴀᴅᴅʏ...!😪😒")
        try:
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ok = await event.reply(f"» __ᴇᴋ ɴᴀʏᴀ⚡️ᴘʀᴏғᴇssᴏʀ's⚡️ʙᴇᴛᴀ ᴀᴅᴅ ʜᴏ ʀʜᴀ...__")
        mks = "SUDO_USER"
        target = ""
        ...
        if len(sudousers) > 0:
            ...
            newsudo = f"{target}"
        await ok.edit(f"» **ɴᴇᴡ⚡️ᴘʀᴏғᴇssᴏʀ's ⚡️ʙᴇᴛᴀ**: `{target}`\n» `ʙᴏᴛ ғɪʀ sᴇ sᴜʀᴜ ʜᴏ ʀʜᴀ...`")
